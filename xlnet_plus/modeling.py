...
def transformer_xl(inp_k, n_token, n_layer, d_model, n_head,
  ...
  with tf.variable_scope(scope):
    ...
    intermediate_output = tf.concat(intermediate_output_list, axis=-2)

    return output, new_mems, lookup_table, intermediate_output
...
def entangle_sequence(output, intermediate_output, input_mask,
                      n_layer, d_model, n_head, d_head,
                      dropout, dropatt, is_training, initializer,
                      scope=None, reuse=None, use_proj=True):

  """
      Different downstream tasks may or may not share the same parameters
      to entangle the sequence features.

      If shared, one can keep the `scope` to the default value `None`.
      Otherwise, one should specify a different `scope` for each task.
  """

  with tf.variable_scope(scope, 'sequence_entanglement', reuse=reuse):
    data_shape = tf.shape(output)
    seq_len = data_shape[0]
    bsz = data_shape[1]
    
    q = output # [l,b,h]
    q_mask = 1 - input_mask # [l,b]
    
    k = tf.transpose(intermediate_output, perm=[2,0,1,3]) # [l,b,n,h] --> [n,l,b,h]
    k = tf.reshape(k, shape=[n_layer * seq_len, bsz, -1]) # [n,l,b,h] --> [n*l,b,h]
    k_mask = tf.expand_dims(input_mask, axis=0) # [l,b] --> [1,l,b]
    k_mask = tf.concat([k_mask] * n_layer, axis=0) # [1,l,b] * n --> [n,l,b]
    k_mask = 1 - tf.reshape(k_mask, shape=[n_layer * seq_len, bsz]) # [n,l,b] --> [n*l,b]
    
    v = k
    attn_mask = 1 - tf.einsum('ib,jb->ijb', q_mask, k_mask)
    attn_mask = tf.expand_dims(attn_mask, axis=-1)

    entanglement = multihead_attn(q, k, v, attn_mask,
                                  d_model, n_head, d_head, dropout, dropatt,
                                  is_training, initializer, residual=False) # [l,b,h]

    # use another projection as in BERT
    if use_proj:
      entanglement = tf.layers.dense(
          entanglement,
          d_model,
          activation=tf.tanh,
          kernel_initializer=initializer,
          name='entanglement') # [l,b,h]

    # dropout
    entanglement = tf.layers.dropout(
        entanglement, dropout, training=is_training,
        name='dropout') # [l,b,h]
