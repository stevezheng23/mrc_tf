...
class XLNetModel(object):
  ...
  def __init__(self, xlnet_config, run_config, input_ids, seg_ids, input_mask,
    ...
    with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
      (self.output, self.new_mems, self.lookup_table,
          self.intermediate_output) = modeling.transformer_xl(**tfm_args)
  ...
  def get_entangled_output(self, use_proj=True):
    """
    Args:
      use_proj: bool, whether to use a linear projection during entanglement.

    Returns:
      float32 Tensor in shape [len, bsz, d_model], the entangled representation.
    """
    ...
    with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
      entanglement = modeling.entangle_sequence(
          output=self.output,
