...
class XLNetModelBuilder(object):
    ...
    def _create_model(self,
        ...
        sequence_result = tf.transpose(model.get_sequence_output(), perm=[1,0,2])                                    # [l,b,h] --> [b,l,h]
        entangled_result = tf.transpose(model.get_entangled_output(), perm=[1,0,2])                                  # [l,b,h] --> [b,l,h]
        output_result = tf.concat([sequence_result, entangled_result], axis=-1)                            # [b,l,h], [b,l,h] --> [b,l,2h]
        
        predicts = {}
        with tf.variable_scope("mrc", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("start", reuse=tf.AUTO_REUSE):
                start_result = output_result                                                                                    # [b,l,2h]
                ...
                start_result = tf.layers.dense(start_result, units=1, activation=None,
                    ...
                    kernel_regularizer=None, bias_regularizer=None, trainable=True, name="start_project")           # [b,l,2h] --> [b,l,1]
            ...
            with tf.variable_scope("end", reuse=tf.AUTO_REUSE):
                if is_training:
                    ...
                    feat_result = tf.matmul(start_index, output_result)                                   # [b,1,l], [b,l,2h] --> [b,1,2h]
                    feat_result = tf.tile(feat_result, multiples=[1,seq_len,1])                                    # [b,1,2h] --> [b,l,2h]
                    
                    end_result = tf.concat([output_result, feat_result], axis=-1)                         # [b,l,2h], [b,l,h] --> [b,l,3h]
                    end_result_mask = 1 - p_mask                                                                                   # [b,l]
                    
                    end_result = tf.layers.dense(end_result, units=self.model_config.d_model, activation=tf.tanh,
                        use_bias=True, kernel_initializer=initializer, bias_initializer=tf.zeros_initializer,
                        kernel_regularizer=None, bias_regularizer=None, trainable=True, name="end_modeling")        # [b,l,3h] --> [b,l,h]
                    ...
                    feat_result = tf.matmul(start_index, output_result)                                   # [b,k,l], [b,l,2h] --> [b,k,2h]
                    feat_result = tf.expand_dims(feat_result, axis=1)                                            # [b,k,2h] --> [b,1,k,2h]
                    feat_result = tf.tile(feat_result, multiples=[1,seq_len,1,1])                              # [b,1,k,2h] --> [b,l,k,2h]
                    
                    end_result = tf.expand_dims(output_result, axis=-2)                                          # [b,l,2h] --> [b,l,1,2h]
                    end_result = tf.tile(end_result, multiples=[1,1,FLAGS.start_n_top,1])                      # [b,l,1,2h] --> [b,l,k,2h]
                    end_result = tf.concat([end_result, feat_result], axis=-1)                      # [b,l,k,2h], [b,l,k,h] --> [b,l,k,3h]
                    ...
                    end_result = tf.layers.dense(end_result, units=self.model_config.d_model, activation=tf.tanh,
                        use_bias=True, kernel_initializer=initializer, bias_initializer=tf.zeros_initializer,
                        kernel_regularizer=None, bias_regularizer=None, trainable=True, name="end_modeling")    # [b,l,k,3h] --> [b,l,k,h]
            ...
            with tf.variable_scope("answer", reuse=tf.AUTO_REUSE):
                ...
                answer_feat_result = tf.matmul(tf.expand_dims(start_prob, axis=1), output_result)           # [b,l], [b,l,2h] --> [b,1,2h]
                answer_output_result = tf.matmul(answer_cls_index, output_result)                         # [b,1,l], [b,l,2h] --> [b,1,2h]
                
                answer_result = tf.concat([answer_feat_result, answer_output_result], axis=-1)           # [b,1,2h], [b,1,2h] --> [b,1,4h]
                answer_result = tf.squeeze(answer_result, axis=1)                                                    # [b,1,4h] --> [b,4h]
                
                answer_result = tf.layers.dense(answer_result, units=self.model_config.d_model, activation=tf.tanh,
                    use_bias=True, kernel_initializer=initializer, bias_initializer=tf.zeros_initializer,
                    kernel_regularizer=None, bias_regularizer=None, trainable=True, name="answer_modeling")             # [b,4h] --> [b,h]
