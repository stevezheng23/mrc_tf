...
flags.DEFINE_float("kd_temperature", default=1.0, help="Temperature for knowledge distillation")
...
class OutputResult(object):
    ...
    def __init__(self,
        ...
        self.unk_kd_prob = unk_kd_prob
        self.yes_kd_prob = yes_kd_prob
        self.no_kd_prob = no_kd_prob
        self.num_kd_probs = num_kd_probs
        self.opt_kd_probs = opt_kd_probs
        self.start_kd_probs = start_kd_probs
        self.end_kd_probs = end_kd_probs
    ...
    def _get_example(self,
        ...
        for data in data_list:
            ...
            if "targets" in data and data["targets"]:
                ...
                targets = [{"is_valid": False, "turn_id": answer["turn_id"]} for answer in answers]
    ...
    def _create_model(self,
        ...
        with tf.variable_scope("mrc", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("start", reuse=tf.AUTO_REUSE):
                ...
                start_kd_result = start_result / self.kd_temperature
                start_kd_result = self._generate_masked_data(start_kd_result, start_result_mask)
                start_kd_probs = tf.nn.softmax(start_kd_result, axis=-1)
                ...
                if not is_training:
                    ...
                    predicts["start_kd_probs"] = start_kd_probs
            ...
            with tf.variable_scope("end", reuse=tf.AUTO_REUSE):
                if is_training:
                    ...
                    end_kd_result = end_result / self.kd_temperature
                    end_kd_result = self._generate_masked_data(end_kd_result, end_result_mask)
                    end_kd_probs = tf.nn.softmax(end_kd_result, axis=-1)
                else:
                    ...
                    # During kd inference, compute the end logits based on the ground truth of the start position
                    start_index = self._generate_onehot_label(tf.expand_dims(start_positions, axis=-1), seq_len)         # [b] --> [b,1,l]
                    feat_result = tf.matmul(start_index, output_result)                                     # [b,1,l], [b,l,h] --> [b,1,h]
                    feat_result = tf.tile(feat_result, multiples=[1,seq_len,1])                                      # [b,1,h] --> [b,l,h]
                    ...
                    end_result = tf.contrib.layers.layer_norm(end_result, center=True, scale=True, activation_fn=None,
                        reuse=True, begin_norm_axis=-1, begin_params_axis=-1, trainable=True, scope="end_norm")      # [b,l,h] --> [b,l,h]
                    ...
                    end_kd_result = end_result / self.kd_temperature
                    end_kd_result = tf.squeeze(end_kd_result, axis=-1)                                                 # [b,l,1] --> [b,l]
                    end_kd_result = self._generate_masked_data(end_kd_result, end_result_mask)                    # [b,l], [b,l] --> [b,l]
                    end_kd_probs = tf.nn.softmax(end_kd_result, axis=-1)                                                           # [b,l]
                    predicts["end_kd_probs"] = end_kd_probs
            ...
            with tf.variable_scope("answer", reuse=tf.AUTO_REUSE):
                ...
                with tf.variable_scope("unk", reuse=tf.AUTO_REUSE):
                    ...
                    unk_kd_result = unk_result / self.kd_temperature
                    unk_kd_result = self._generate_masked_data(unk_kd_result, unk_result_mask)
                    unk_kd_prob = tf.nn.softmax(unk_kd_result, axis=-1)
                    
                    predicts["unk_prob"] = unk_prob
                    predicts["unk_kd_prob"] = unk_kd_prob
                ...
                with tf.variable_scope("yes", reuse=tf.AUTO_REUSE):
                    ...
                    yes_kd_result = yes_result / self.kd_temperature
                    yes_kd_result = self._generate_masked_data(yes_kd_result, yes_result_mask)
                    yes_kd_prob = tf.nn.softmax(yes_kd_result, axis=-1)
                    
                    predicts["yes_prob"] = yes_prob
                    predicts["yes_kd_prob"] = yes_kd_prob
                ...
                with tf.variable_scope("no", reuse=tf.AUTO_REUSE):
                    ...
                    no_kd_result = no_result / self.kd_temperature
                    no_kd_result = self._generate_masked_data(no_kd_result, no_result_mask)
                    no_kd_prob = tf.nn.softmax(no_kd_result, axis=-1)
                    
                    predicts["no_prob"] = no_prob
                    predicts["no_kd_prob"] = no_kd_prob
                ...
                with tf.variable_scope("num", reuse=tf.AUTO_REUSE):
                    ...
                    num_kd_result = num_result / self.kd_temperature
                    num_kd_result = self._generate_masked_data(num_kd_result, num_result_mask)
                    num_kd_probs = tf.nn.softmax(num_kd_result, axis=-1)
                    
                    predicts["num_probs"] = num_probs
                    predicts["num_kd_probs"] = num_kd_probs
            ...
            if is_training:
                ...
                output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                    ...
                    predictions={
                        ...
                        "unk_kd_prob": predicts["unk_kd_prob"],
                        "yes_kd_prob": predicts["yes_kd_prob"],
                        "no_kd_prob": predicts["no_kd_prob"],
                        "num_kd_probs": predicts["num_kd_probs"],
                        "opt_kd_probs": predicts["opt_kd_probs"],
                        "start_kd_probs": predicts["start_kd_probs"],
                        "end_kd_probs": predicts["end_kd_probs"]
                    },
                    scaffold_fn=scaffold_fn)
...
class XLNetPredictProcessor(object):
    ...
    def process(self,
        ...
        for (example_idx, example) in enumerate(examples):
            ...
            example_unk_kd_score = None
            example_yes_kd_score = None
            example_no_kd_score = None
            example_num_kd_probs = None
            example_opt_kd_probs = None
            ...
            for example_feature in example_features:
                ...
                unk_score = float(example_result.unk_prob)
                unk_kd_score = float(example_result.unk_kd_prob)
                if example_unk_score > unk_score:
                    example_unk_score = unk_score
                    example_unk_kd_score = unk_kd_score
                
                yes_score = float(example_result.yes_prob)
                yes_kd_score = float(example_result.yes_kd_prob)
                if example_yes_score < yes_score:
                    example_yes_score = yes_score
                    example_yes_kd_score = yes_kd_score
                
                no_score = float(example_result.no_prob)
                no_kd_score = float(example_result.no_kd_prob)
                if example_no_score < no_score:
                    example_no_score = no_score
                    example_no_kd_score = no_kd_score
                
                num_probs = [float(num_prob) for num_prob in example_result.num_probs]
                num_kd_probs = [float(num_kd_prob) for num_kd_prob in example_result.num_kd_probs]
                ...
                if example_num_score < num_score:
                    ...
                    example_num_kd_probs = num_kd_probs
                
                opt_probs = [float(opt_prob) for opt_prob in example_result.opt_probs]
                opt_kd_probs = [float(opt_kd_prob) for opt_kd_prob in example_result.opt_probs]
                ...
                if example_opt_score < opt_score:
                    ...
                    example_opt_kd_probs = opt_kd_probs
                
                start_kd_probs = [float(start_kd_prob) for start_kd_prob in example_result.start_kd_probs]
                end_kd_probs = [float(end_kd_prob) for end_kd_prob in example_result.end_kd_probs]
                ...
                for i in range(self.start_n_top):
                    ...
                    for j in range(self.end_n_top):
                        ...
                        example_all_predicts.append({
                            ...
                            "start_kd_probs": start_kd_probs,
                            "end_kd_probs": end_kd_probs
                        })
            ...
            example_top_predicts = [{
                "predict_text": example_top_predict["predict_text"],
                "predict_score": example_top_predict["predict_score"]
            } for example_top_predict in example_top_predicts]
            ...
            predict_summary_list.append({
                ...
                "unk_kd_score": example_unk_kd_score,
                "yes_kd_score": example_yes_kd_score,
                "no_kd_score": example_no_kd_score,
                "num_kd_probs": example_num_kd_probs,
                "opt_kd_probs": example_opt_kd_probs,
                "start_kd_probs": example_best_predict["start_kd_probs"],
                "end_kd_probs": example_best_predict["end_kd_probs"],
            })
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        predict_results = [OutputResult(
            ...
            unk_kd_prob=result["unk_kd_prob"],
            yes_kd_prob=result["yes_kd_prob"],
            no_kd_prob=result["no_kd_prob"],
            num_kd_probs=result["num_kd_probs"].tolist(),
            opt_kd_probs=result["opt_kd_probs"].tolist(),
            start_kd_probs=result["start_kd_probs"].tolist(),
            end_kd_probs=result["end_kd_probs"].tolist()
        ) for result in results]
